...
import { deleteFinanceSharingById } from 'apiSdk/finance-sharings';
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'finance_sharing', 'membership'],
      }),
  );

  const finance_sharingHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('finance_sharing', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/finance-sharings/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.finance_sharing?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/finance-sharings/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('finance_sharing', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/finance-sharings/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('finance_sharing', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    finance_sharingHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('membership', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/memberships/create?organization_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
